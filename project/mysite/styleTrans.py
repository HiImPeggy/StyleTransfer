...
def generate_out_img_name(input_name):
    '''
    ...
    return 'transfered'+input_name

def save_and_maybe_display(optimizing_img, dump_path, config, img_id, num_of_iterations, input_name):
    '''
    ...
    if img_id == num_of_iterations-1 :
        ...
        out_img_name = str(img_id).zfill(img_format[0]) + img_format[1] if saving_freq != -1 else generate_out_img_name(input_name)
...
def neural_style_transfer(config, input_path, trans_path):
    '''
    The main Neural Style Transfer method.
    '''
    content_img_path = './static/media/uploads/' + input_path
    style_img_path = '.' + trans_path
    dump_path = './static/media/transfered'
    ...
    def closure():
        ...
        with torch.no_grad():
            ...
            save_and_maybe_display(optimizing_img, dump_path, config, cnt, num_of_iterations, input_path)
        cnt += 1
        return total_loss
    optimizer.step(closure)
    return dump_path

def main(input_path, trans_path):

    output_img_dir = './static/media/transfered'
    img_format = (4, '.jpg')

    optimization_config = {'height': 400, 'content_weight': 100000.0, 'style_weight': 80000.0, 'tv_weight': 1.0}
    
    optimization_config['output_img_dir'] = output_img_dir
    optimization_config['img_format'] = img_format

    results_path = neural_style_transfer(optimization_config, input_path, trans_path)
